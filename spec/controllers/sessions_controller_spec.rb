...
RSpec.describe SessionsController, type: :controller do
  let(:my_user) {User.create!(name: "Brian", email: "briansmont830@gmail.com", password: "password") }
  ...
  describe "POST sessions" do
    ...
    it "doesn't add a user id to session because of a missing password" do
      post :create, session: {email: my_user.email}
      expect(session[:user_id]).to be_nil
    end
    it "flashes #error with bad email address" do
      post :create, session: {email: "doesn't exist"}
